...
*/
Wtf.EmployerContributionForm=function(config){
    config.border=false;
    config.layout="fit";
    Wtf.EmployerContributionForm.superclass.constructor.call(this,config);
    Wtf.apply(this, config);
}
Wtf.extend(Wtf.EmployerContributionForm,Wtf.Panel,{
    initComponent:function(config){
        Wtf.EmployerContributionForm.superclass.initComponent.call(this,config);
        this.earncomp = 0;
        this.deduccomp = 0;
        this.netcomp = 0;
        this.fieldswage=[
        ...
        this.Wtypecombo= new Wtf.form.ComboBox({
            ...
            fieldLabel:WtfGlobal.getLocaleText("hrms.payroll.EmployerContributionTypeName"),
            displayField:'type',
            mode: 'local',
            valueField:'type',
            forceSelection:true,
...
//            }),
            this.expander,{
                header: WtfGlobal.getLocaleText("hrms.payroll.EmployerContributionType"),
                dataIndex: 'type',
        ...
        this.grid = new Wtf.grid.EditorGridPanel({
            ...
            height:440,
            clicksToEdit:1,
            title:WtfGlobal.getLocaleText("hrms.payroll.EmployerContribution"),
            plugins:this.expander,
            viewConfig: {
                forceFit: true,
                emptyText:WtfGlobal.emptyGridRenderer(WtfGlobal.getLocaleText("hrms.payroll.Noemployercontributionassignedforcurrentsalarytemplate"))
            },
            cm:this.cmwage
        });
        this.grid.on("validateedit",function(e)
        {
            ...
            if(this.grid.getStore().find('type',e.value)==-1){
                if(this.insertratecomputeon=="0"){//current deduction
                        var idsplit = this.id.split("addempcontrib");
                        if(idsplit.length>0){
                ...
                if(index==-1 && subexpr[cnt]!=""){
                     var ind = this.storewage.find('id',wgid);
                      if(ind>-1){
                          ...
                          if(comborec.get("comp")=="empcontrib")
                                this.grid.getStore().insert(0,comborec);
                          else if(comborec.get("comp")=="wage"){
                                idsplit = this.id.split("addempcontrib");
                                if(idsplit.length>0){
