...
Wtf.lib.Ajax.combinereq = function(){
    ...
    while(this._queue.length>0){
        ...
        if(ele.uri=="General/getData.dsh"){
            newparam.push(ele);
            ele = null;
            continue
        }
    ...
    if(param.length>0){
        if(ele){
            if(this._activeRequests < this.Queue_Size){
                this._asyncRequest(ele.o, ele.method, "General/getData.dsh", ele.callback, Wtf.urlEncode(param));
            }else{
                this._queue.push({
                   o : ele.o,
                   method: ele.method,
...
Wtf.lib.Ajax._processQueue = function(flag)
{
    ...
    if(flag==1){
        to = this._queue[0];
        if(to&&to.uri=="General/getData.dsh"){
            this.simplerequest();
            return;
        }
    }else{
