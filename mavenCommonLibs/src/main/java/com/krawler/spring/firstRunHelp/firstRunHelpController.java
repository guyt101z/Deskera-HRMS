 ...
 * @author Karthik
 */
public class firstRunHelpController extends MultiActionController {
    ...
    public JSONObject getComponentsJson(List ll) throws ServiceException {
        JSONObject jobj = new JSONObject();
        JSONArray jarr = new JSONArray();
        try {
            Iterator ite = ll.iterator();
    ...
    public ModelAndView getComponents(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            jobj = getComponentsJson(kmsg.getEntityList());
            jobj.put("totalCount", kmsg.getRecordTotalCount());
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
