 ...
 * @author Karthik
 */
public class firstRunHelpDAOImpl implements firstRunHelpDAO {
    ...
    public KwlReturnObject getComponents(String modName) throws ServiceException {
        ...
        try {
            ...
            ll = HibernateUtil.executeQuery(hibernateTemplate, Hql, new Object[]{modName});
            dl = ll.size();
        } catch (Exception e) {
    ...
    public KwlReturnObject saveCompDetails(HashMap<String, Object> requestParams) throws ServiceException {
        ...
        try {
            if (requestParams.containsKey("id") && !requestParams.get("id").toString().equals("")) {
                EditHelp help = (EditHelp) hibernateTemplate.load(EditHelp.class, requestParams.get("id").toString());
